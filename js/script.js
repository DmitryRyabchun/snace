$(document).ready(function () {
    ...
    function createField(width, height) {
        for (var i = 0; i < height; i++) {
            $('.field').append(
                '<div id = "field-line_' + i + '" class="field-line"></div>'
            );
            for (var j = 0; j < width; j++) {
                $('#field-line_' + i).append(
                    '<div id = "block_' + i + '_' + j + '"></div>'
                );
            }
        }
    }
    ...
    //вывод данных о последней игре и лучшей игре
    function statusBar() {
        ...
        if (counter > best_game) {
            best_game = counter;
            $('.best_game').html('<span>' + best_game + '</span>');
        }
        console.log(last_game);
        console.log(counter);
        last_game = counter;
        if (counter !== 0) { //исправляет баг с двойной перезагрузкой
    ...
    function Overstepping() {
        if (snake_x[length - 1] === width) {
            snake_x[length - 1] = 0;
            $('#block_' + snake_x[length - 1] + '_' + snake_y[length - 1]).css(
                'background',
                color_snace
            );
